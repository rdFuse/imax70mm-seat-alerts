...
# NOTE: this runs as a single sweep per invocation (see GitHub Actions section in
# the README) -- there is no in-script polling loop or rediscovery interval.
# How OFTEN it runs is controlled by the GitHub Actions schedule/external trigger,
# not by any setting in this file.
...
# State file: each GitHub Actions run is a fresh process, so dedup/phantom memory
# is saved here at the end of every run and reloaded at the start of the next.
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "state_citywalk.json")

# Queue detection: AMC serves a tiny waiting-room page during high-demand drops.
# Detect it (page far smaller than a real one + no 70mm content) and skip this
# run cleanly rather than alerting on bad/empty data -- the next scheduled run
# will pick it up once the real page is back.
QUEUE_MIN_PAGE_SIZE = 120000

# Phantom filter (added here to match the NYC watcher's behavior -- this file
# didn't have it before). Some seats show "available" but aren't purchasable and
# sit open indefinitely; a seat open continuously past PHANTOM_AFTER_SECONDS is
# muted, and re-armed for one more chance every PHANTOM_RECHECK_SECONDS. A seat
# that disappears and reopens later is treated as a fresh opening.
PHANTOM_AFTER_SECONDS = 600
PHANTOM_RECHECK_SECONDS = 1800
...
def date_range():
    start = dt.date.fromisoformat(START_DATE) if START_DATE else dt.datetime.now(ZoneInfo(THEATRE_TZ)).date()
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        # Single probe for the queue (don't burn the job's time budget waiting on
        # it -- if it's up, exit cleanly and let the next scheduled run try).
        probe_html = ""
        try:
        ...
        for fut in asyncio.as_completed(tasks):
            ...
            if not is_imax70:
                print(f"[{stamp}] show {sid}: skipped (not Odyssey IMAX 70mm)")
                continue
            n = len(open_list)
            if n == 0:
                note = ""
