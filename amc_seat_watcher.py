...
PHANTOM_AFTER_SECONDS = 120         # 2 min open non-stop -> treat as phantom, mute it
PHANTOM_RECHECK_SECONDS = 1800      # after 30mins stuck, re-arm for one more alert
...
async def run():
    ...
    alerted_at = {}
    open_since = {}     # {sid: {seat: time it started its current continuous-open streak}}
    shows = {}
    last_discovery = 0.0
    ...
    async with async_playwright() as p:
        ...
        # Wait out AMC's queue before the first sweep (returns as soon as it clears).
        await wait_out_queue(context, sem)

        while True:
            if time.time() - last_discovery > REDISCOVER_SECONDS:
                shows = await discover(context, sem)
                # If discovery came back empty, the queue may have popped up again
                # mid-run; wait it out and re-discover rather than sweeping blind.
                if not shows:
                    await wait_out_queue(context, sem)
                    shows = await discover(context, sem)
